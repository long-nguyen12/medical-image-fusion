...
def loss_int(fusion_image, images_ir, images_vi):
    g_loss_int = torch.mean(torch.square(fusion_image - images_ir)) + 5 * torch.mean(
        torch.square(fusion_image - images_vi)
    )
...
def loss_grad(fusion_image, images_ir, images_vi):
    ...
    Image_vi_score_2 = torch.sign(
        Image_vi_weight_lowpass
        - torch.minimum(Image_vi_weight_lowpass, Image_ir_weight_lowpass)
    )

    Image_vi_score = Image_vi_score_1 * Image_vi_score_2
    Image_ir_score = 1 - Image_vi_score
    g_loss_grad = torch.mean(
        Image_ir_score * torch.square(gradient(fusion_image) - gradient(images_ir))
    ) + torch.mean(
        Image_vi_score * torch.square(gradient(fusion_image) - gradient(images_vi))
    )
...
def gradient(image):
    ...
    # print(h, w)
    img = image
    k = torch.tensor([0.0, 1.0, 0.0, 1.0, -4.0, 1.0, 0.0, 1.0, 0.0], dtype=torch.float)
...
def loss_de(sept_ir, sept_vi, images_ir, images_vi):
    g_loss_sept = torch.mean(torch.square(sept_ir - images_ir)) + torch.mean(
        torch.square(sept_vi - images_vi)
    )
    return g_loss_sept
...
def loss_total(fusion_image, sept_ir, sept_vi, images_ir, images_vi):
    g_loss = (
        10 * loss_int(fusion_image, images_ir, images_vi)
