...
def _ssim(img1, img2, img3, window, window_size, channel, size_average=True):
    ...
    sigma3_sq = (
        F.conv2d(img3 * img3, window, padding=window_size // 2, groups=channel) - mu3_sq
    )
    sigma12 = (
    ...
    sigma13 = (
        F.conv2d(img1 * img3, window, padding=window_size // 2, groups=channel)
        - mu1_mu3
    )
    ...
    if size_average:
        ssim_map_mu = torch.where(
            torch.abs((mu2)) >= torch.abs((mu3)), ssim_map_12, ssim_map_13
        )
        return ssim_map_mu.mean()
    else:
        ssim_map_mu = torch.where(
            torch.abs((mu2)) >= torch.abs((mu3)), ssim_map_12, ssim_map_13
        )
...
def SSIM(img1, img2, window_size=11, size_average=True):
    ...
    C1 = 0.01**2
    C2 = 0.03**2
    ssim_map_12 = ((2 * mu1_mu2 + C1) * (2 * sigma12 + C2)) / (
