...
def ssim_f(img1, img2, fused, measure):
    return 0.5 * (measure(img1, fused) + measure(img2, fused))
...
def Qmi(img1, img2, fused):
    ...
    if len(img1.shape) != 2 or len(img2.shape) != 2:
        raise TypeError(
            f"Expected source images to be in grayscale(2-d), "
            f"but got {img1.shape} and {img2.shape}"
        )
    if img1.shape != img2.shape:
        raise TypeError(
            f"Expected tensors of equal shapes, "
            f"but got {img1.shape} and {img2.shape}"
        )
    I_AF = MI(img1, fused)
    I_BF = MI(img2, fused)
    ...
    Qmi = 2.0 * (I_AF / (EN_A + EN_F) + I_BF / (EN_B + EN_F))
    return Qmi
